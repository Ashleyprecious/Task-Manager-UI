...
// ─── extractUser — maps API response → internal user shape ───────────────────
function extractUser(data) {
  ...
  return {
    ...
    // profile_photo is the server field; fall back to avatar / profile_picture
    avatar:       API_BASE +(u.profile_photo || u.avatar   || u.profile_picture || null),
...
function useToken() {
  ...
  const clearToken = () => { ["tm_token", "tm_user"].forEach(k => localStorage.removeItem(k)); setT(""); };
...
function Spinner({ size = 18, color = "currentColor" }) {
  return (
    <span style={{ display: "inline-block", width: size, height: size, flexShrink: 0, border: `2px solid ${color}30`, borderTopColor: color, borderRadius: "50%", animation: "spin 0.65s linear infinite" }} />
  );
}
...
function PrimaryBtn({ children, loading, type = "button", onClick, fullWidth = true, color, danger }) {
  ...
  return (
    <button type={type} onClick={onClick} disabled={loading}
      style={{ width: fullWidth ? "100%" : "auto", padding: "12px 24px", borderRadius: "var(--radius)", background: loading ? "#ccc" : bg, color: "#fff", fontSize: 14, fontWeight: 700, fontFamily: "var(--font-body)", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background var(--trans),transform var(--trans)" }}
...
function ConfirmDialog({ title, message, confirmLabel = "Confirm", danger = true, onConfirm, onCancel, loading, children }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(10,10,15,0.55)", backdropFilter: "blur(8px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 600, padding: 24, animation: "fadeIn 0.15s ease" }}
      ...
      <div style={{ background: "#fff", borderRadius: "var(--radius-xl)", width: "100%", maxWidth: 420, border: "1px solid var(--mist)", boxShadow: "0 24px 64px rgba(0,0,0,0.2)", animation: "scaleIn 0.2s ease", overflow: "hidden" }}>
        ...
        <div style={{ padding: "16px 26px 24px", display: "flex", gap: 10 }}>
          <button onClick={onCancel} style={{ flex: 1, padding: "10px", borderRadius: "var(--radius-lg)", fontSize: 13, fontWeight: 500, color: "#777", border: "1px solid var(--mist2)", background: "transparent", transition: "border-color var(--trans)", fontFamily: "var(--font-body)" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = "var(--ink)"} onMouseLeave={e => e.currentTarget.style.borderColor = "var(--mist2)"}>Cancel</button>
          <button onClick={onConfirm} disabled={loading} style={{ flex: 2, padding: "10px", borderRadius: "var(--radius-lg)", fontSize: 13, fontWeight: 700, color: "#fff", background: loading ? "#ccc" : danger ? "#dc2626" : "var(--rust)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background var(--trans)", fontFamily: "var(--font-body)" }}
            onMouseEnter={e => { if (!loading) e.currentTarget.style.background = danger ? "#b91c1c" : "var(--rust-light)"; }}
            onMouseLeave={e => { if (!loading) e.currentTarget.style.background = danger ? "#dc2626" : "var(--rust)"; }}>
...
// ─── Avatar ───────────────────────────────────────────────────────────────────
// Reads photo URL from user.avatar (set from profile_photo on login/upload).
// No localStorage image cache — the server URL is the source of truth.
      ...
      <div style={{ position: "relative", flexShrink: 0 }}>
        <svg width="124" height="124" viewBox="0 0 124 124" style={{ transform: "rotate(-90deg)" }}>
          ...
          {arcs.map((a, i) => a.count > 0 && (
            <circle key={i} cx="62" cy="62" r={R} fill="none" stroke={a.color} strokeWidth="14"
...
function StatCard({ label, value, iconName, color, borderColor, onClick }) {
  ...
  useEffect(() => {
    setShown(0);
    const step = Math.ceil((value || 0) / 18) || 1; let cur = 0;
    const t = setInterval(() => { cur += step; if (cur >= (value || 0)) { setShown(value || 0); clearInterval(t); } else setShown(cur); }, 28);
...
function Pagination({ page, total, perPage, onChange }) {
  ...
  const btnStyle = (active, disabled) => ({
    minWidth: 32, height: 32, padding: "0 6px", borderRadius: "var(--radius)",
  ...
  return (
    <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 20, paddingTop: 16, borderTop: "1px solid var(--mist)", flexWrap: "wrap", gap: 10 }}>
      <span style={{ fontSize: 12, color: "#aaa" }}>
        Showing <strong style={{ color: "var(--ink)" }}>{start}–{end}</strong> of{" "}
        <strong style={{ color: "var(--ink)" }}>{total}</strong> tasks
      </span>
      <div style={{ display: "flex", alignItems: "center", gap: 4 }}>
        <button onClick={() => page > 1 && onChange(page - 1)} disabled={page === 1} style={btnStyle(false, page === 1)}
          onMouseEnter={e => { if (page > 1) { e.currentTarget.style.background = "var(--mist)"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}
          onMouseLeave={e => { if (page > 1) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}>
          <Icon name="chevron-left" size={13} color="currentColor" strokeWidth={2} />
        </button>
        {getPages().map((p, i) =>
          p === "…"
            ? <span key={`e${i}`} style={{ minWidth: 32, height: 32, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, color: "#ccc" }}>…</span>
            : <button key={p} onClick={() => onChange(p)} style={btnStyle(p === page, false)}
                onMouseEnter={e => { if (p !== page) { e.currentTarget.style.background = "var(--mist)"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}
                onMouseLeave={e => { if (p !== page) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}>
                {p}
              </button>
        )}
        <button onClick={() => page < totalPages && onChange(page + 1)} disabled={page === totalPages} style={btnStyle(false, page === totalPages)}
          onMouseEnter={e => { if (page < totalPages) { e.currentTarget.style.background = "var(--mist)"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}
          onMouseLeave={e => { if (page < totalPages) { e.currentTarget.style.background = "transparent"; e.currentTarget.style.borderColor = "var(--mist2)"; } }}>
          <Icon name="chevron-right" size={13} color="currentColor" strokeWidth={2} />
        </button>
      </div>
    </div>
  );
...
// ─── Photo Upload Modal ───────────────────────────────────────────────────────
// Sends multipart FormData to /api/users/upload-photo.
// Calls onSave(photoUrl) with the URL returned by the server.
function PhotoUploadModal({ onClose, onSave, showToast, token, currentAvatar }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Accept common response shapes for the returned photo URL
      const photoUrl =
        data?.user?.profile_photo ||
        data?.data?.profile_photo ||
        data?.profile_photo       ||
        data?.url                 ||
        preview; // graceful fallback to local preview
  ...
  const handleRemove = async () => {
    try {
      await apiFetch("/api/users/remove-photo", { method: "POST" }, token);
    } catch {
      // Silently continue even if endpoint doesn't exist
    }
      ...
      <div style={{ background: "#fff", borderRadius: "var(--radius-xl)", width: "100%", maxWidth: 440, border: "1px solid var(--mist)", boxShadow: "0 24px 64px rgba(0,0,0,0.18)", animation: "scaleIn 0.2s ease", overflow: "hidden" }}>
        ...
        <div style={{ padding: "28px 24px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "center", marginBottom: 24 }}>
            <div style={{ width: 110, height: 110, borderRadius: "50%", overflow: "hidden", background: "linear-gradient(135deg,var(--rust),var(--rust-light))", display: "flex", alignItems: "center", justifyContent: "center", border: "3px solid var(--mist2)", boxShadow: "0 4px 20px var(--shadow)", marginBottom: 12 }}>
              {(preview || currentAvatar)
              
                ? <img src={preview || currentAvatar} alt="" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                : <Icon name="user" size={40} color="rgba(255,255,255,0.6)" strokeWidth={1.5} />
              }
            </div>
            <div style={{ fontSize: 12, color: "#aaa" }}>
              {preview ? "New photo preview" : currentAvatar ? "Current photo" : "No photo set"}
            </div>
          </div>
          ...
          <div onClick={() => fileRef.current?.click()}
            style={{ border: "2px dashed var(--mist2)", borderRadius: "var(--radius-lg)", padding: "24px", textAlign: "center", cursor: "pointer", transition: "all var(--trans)", marginBottom: 16, background: preview ? "rgba(201,70,10,0.02)" : "transparent" }}
          ...
          <div style={{ display: "flex", gap: 10 }}>
            {currentAvatar && !preview && (
              <button onClick={handleRemove}
                style={{ flex: 1, padding: "10px", borderRadius: "var(--radius-lg)", fontSize: 13, fontWeight: 600, color: "#dc2626", border: "1px solid #fecaca", background: "#fef2f2", transition: "all var(--trans)", fontFamily: "var(--font-body)" }}
            ...
            <button onClick={handleSave} disabled={!preview || saving}
              style={{ flex: 2, padding: "10px", borderRadius: "var(--radius-lg)", fontSize: 13, fontWeight: 700, color: "#fff", background: (!preview || saving) ? "#ccc" : "var(--rust)", border: "none", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "background var(--trans)", fontFamily: "var(--font-body)" }}
              onMouseEnter={e => { if (preview && !saving) e.currentTarget.style.background = "var(--rust-light)"; }}
              onMouseLeave={e => { if (preview && !saving) e.currentTarget.style.background = "var(--rust)"; }}>
...
function ProfilePanel({ user, setUser, token, onClose, showToast, onLogout }) {
  ...
  useEffect(() => {
    setForm({ first_name: user.first_name || "", last_name: user.last_name || "", email: user.email || "", phone_number: user.phone_number || "" });
  }, [user]);
  ...
  const handlePasswordChange = async e => {
    ...
    setPwLoading(true);
    try {
      await apiFetch("/api/users/change-password", { method: "POST", body: JSON.stringify({ current_password: pwForm.current, new_password: pwForm.newPw }) }, token);
  ...
  const PwField = ({ label, field, placeholder }) => (
    <Field label={label} type={showPw[field] ? "text" : "password"} value={pwForm[field]}
      onChange={e => setPwForm(p => ({ ...p, [field]: e.target.value }))} placeholder={placeholder} error={pwErrors[field]}
      rightEl={
        <button type="button" onClick={() => setShowPw(p => ({ ...p, [field]: !p[field] }))} style={{ color: "#aaa", display: "flex" }}
          onMouseEnter={e => e.currentTarget.style.color = "var(--ink)"} onMouseLeave={e => e.currentTarget.style.color = "#aaa"}>
    ...
    <>
      ...
      <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(500px,95vw)", background: "var(--paper)", borderLeft: "1px solid var(--mist)", zIndex: 301, display: "flex", flexDirection: "column", boxShadow: "-16px 0 56px rgba(10,10,15,0.16)", animation: "slideInPanel 0.28s cubic-bezier(0.4,0,0.2,1)" }}>
        ...
        <div style={{ background: "#fff", borderBottom: "1px solid var(--mist)", padding: "18px 24px", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
          <button onClick={onClose} style={{ width: 30, height: 30, borderRadius: "var(--radius)", display: "flex", alignItems: "center", justifyContent: "center", color: "#aaa", transition: "all var(--trans)" }}
            onMouseEnter={e => { e.currentTarget.style.background = "var(--mist)"; e.currentTarget.style.color = "var(--ink)"; }}
        ...
        <div style={{ background: "linear-gradient(135deg,var(--ink) 0%,#2a2a3a 100%)", padding: "32px 24px 24px", display: "flex", flexDirection: "column", alignItems: "center", gap: 14, flexShrink: 0, position: "relative", overflow: "hidden" }}>
          ...
          <Avatar user={user} size={88} onClick={() => setPhotoModal(true)} showCamera />
          <div style={{ textAlign: "center", position: "relative", zIndex: 1 }}>
            <div style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 22, color: "#fff", marginBottom: 2 }}>{user.first_name} {user.last_name}</div>
            ...
            <button onClick={() => setPhotoModal(true)} style={{ fontSize: 11, color: "rgba(255,255,255,0.5)", background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.12)", padding: "4px 14px", borderRadius: "99px", transition: "all var(--trans)", display: "inline-flex", alignItems: "center", gap: 6 }}
              ...
              <Icon name="camera" size={11} color="rgba(255,255,255,0.6)" strokeWidth={2} /> Change photo
            </button>
          </div>
        </div>
        ...
        <div style={{ display: "flex", borderBottom: "1px solid var(--mist)", background: "#fff", flexShrink: 0, overflowX: "auto" }}>
          {[{ id: "details", label: "Details", icon: "user" }, { id: "edit", label: "Edit", icon: "edit" }, { id: "security", label: "Password", icon: "lock" }, { id: "danger", label: "Account", icon: "shield" }].map(t => (
                  ...
                  <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
                    ...
                    <div style={{ flex: 1 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: "var(--ink)", marginBottom: 6 }}>
                        {user.avatar ? "Photo uploaded" : "No photo set"}
                      </div>
                      <button type="button" onClick={() => setPhotoModal(true)} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, fontWeight: 600, color: "var(--rust)", background: "rgba(201,70,10,0.06)", border: "1px solid rgba(201,70,10,0.2)", padding: "6px 14px", borderRadius: "var(--radius-lg)", transition: "all var(--trans)" }}
                        onMouseEnter={e => e.currentTarget.style.background = "rgba(201,70,10,0.12)"}
            ...
            <div style={{ animation: "slideInRight 0.22s ease" }}>
              <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", border: "1px solid var(--mist)", padding: "18px 18px 14px", marginBottom: 14 }}>
                <div style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 14, marginBottom: 4, paddingBottom: 10, borderBottom: "1px solid var(--mist)", display: "flex", alignItems: "center", gap: 8 }}>
                  <Icon name="lock" size={14} color="var(--rust)" strokeWidth={2} />Change Password
                </div>
                  ...
                  {pwForm.newPw && (
                    <div style={{ marginBottom: 14 }}>
                      ...
                      <div style={{ display: "flex", gap: 4, marginBottom: 4 }}>
                        {[1, 2, 3, 4].map(i => {
                          const score = [pwForm.newPw.length >= 8, /[A-Z]/.test(pwForm.newPw), /\d/.test(pwForm.newPw), /[^a-zA-Z0-9]/.test(pwForm.newPw)].filter(Boolean).length;
            ...
            <div style={{ animation: "slideInRight 0.22s ease" }}>
              <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", border: "1px solid var(--mist)", padding: "18px", marginBottom: 16 }}>
                <div style={{ fontFamily: "var(--font-display)", fontWeight: 600, fontSize: 14, marginBottom: 14, paddingBottom: 10, borderBottom: "1px solid var(--mist)", display: "flex", alignItems: "center", gap: 8 }}>
                  <Icon name="shield" size={14} color="#15803d" strokeWidth={2} />Account Overview
                </div>
              ...
              <div style={{ background: "#fff", borderRadius: "var(--radius-lg)", border: "1.5px solid #fecaca", padding: "18px", marginBottom: 16 }}>
                <div style={{ display: "flex", alignItems: "flex-start", gap: 12, marginBottom: 14 }}>
                  ...
                  <div>
                    ...
                    <div style={{ fontSize: 12, color: "#777", lineHeight: 1.65 }}>Permanently delete your account and all associated data. This action <strong>cannot be undone</strong>.</div>
                  </div>
                </div>
                <button onClick={() => setDeleteConfirm(true)} style={{ width: "100%", padding: "10px 16px", borderRadius: "var(--radius-lg)", fontSize: 13, fontWeight: 700, color: "#dc2626", background: "#fef2f2", border: "1.5px solid #fecaca", display: "flex", alignItems: "center", justifyContent: "center", gap: 8, transition: "all var(--trans)", fontFamily: "var(--font-body)" }}
                  onMouseEnter={e => e.currentTarget.style.background = "#fee2e2"}
                  onMouseLeave={e => e.currentTarget.style.background = "#fef2f2"}>
      ...
      {/* Photo upload modal — passes token & current avatar URL */}
      {photoModal && (
        <PhotoUploadModal
          token={token}
          ...
          onSave={photoUrl => {
            setPhotoModal(false);
            const updated = { ...user, avatar: photoUrl };
            store.set("tm_user", updated);
            setUser(updated);
          }}
        />
      )}
      ...
      {deleteConfirm && (
        <ConfirmDialog
          ...
          message={`This will permanently delete your account (${user.email}) and all your tasks.`}
          confirmLabel={deleteConfirmText === "DELETE"
            ? <><Icon name="trash" size={13} color="#fff" strokeWidth={2} /> Yes, delete forever</>
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)", display: "flex", alignItems: "center", justifyContent: "center", padding: 24, position: "relative" }}>
      ...
      <div style={{ width: "100%", maxWidth: 420, position: "relative", animation: "scaleIn 0.28s ease" }}>
        <button onClick={onBack} style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 13, color: "#999", fontWeight: 500, marginBottom: 32, transition: "color var(--trans)" }}
    ...
    <AuthPage onBack={onBack}>
      ...
      <form onSubmit={submit}>
        ...
        <Field label="Password" type={showPw ? "text" : "password"} value={form.password} onChange={set("password")} required placeholder="••••••••"
          rightEl={
            <button type="button" onClick={() => setShowPw(p => !p)} style={{ color: "#aaa", display: "flex" }}
              onMouseEnter={e => e.currentTarget.style.color = "var(--ink)"}
            ...
            <div style={{ position: "absolute", right: 0, top: 34, background: "#fff", border: "1px solid var(--mist)", borderRadius: "var(--radius-lg)", boxShadow: "0 8px 28px var(--shadow)", zIndex: 50, minWidth: 185, overflow: "hidden", animation: "scaleIn 0.14s ease" }}
              ...
              <div style={{ padding: "7px 7px 4px" }}>
                ...
                {STATUSES.map(st => {
                  ...
                  return (
                    <button key={st} onClick={() => { onStatusChange(task.id, st); setMenuOpen(false); }}
                      style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", textAlign: "left", padding: "7px 9px", fontSize: 13, borderRadius: "var(--radius)", fontWeight: task.status === st ? 700 : 400, color: task.status === st ? m.color : "#555", background: task.status === st ? m.bg : "transparent", transition: "background 0.1s" }}
                      ...
                      <Icon name={m.icon} size={13} color={m.dot} strokeWidth={2} />{st}
              ...
              <div style={{ borderTop: "1px solid var(--mist)", padding: "4px 7px 7px" }}>
                <button onClick={() => { onEdit(task); setMenuOpen(false); }} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 9px", fontSize: 13, borderRadius: "var(--radius)", color: "#555", transition: "background 0.1s" }}
                  ...
                  <Icon name="edit" size={13} color="currentColor" strokeWidth={1.8} /> Edit task
                </button>
                <button onClick={handleDelete} style={{ display: "flex", alignItems: "center", gap: 8, width: "100%", padding: "7px 9px", fontSize: 13, borderRadius: "var(--radius)", color: "#dc2626", transition: "background 0.1s" }}
                  ...
                  onMouseLeave={e => e.currentTarget.style.background = "transparent"}>
                  <Icon name="trash" size={13} color="#dc2626" strokeWidth={1.8} /> Delete task
                </button>
              </div>
            </div>
          )}
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <button onClick={() => onEdit(task)} style={{ width: 28, height: 28, borderRadius: "var(--radius)", display: "flex", alignItems: "center", justifyContent: "center", color: "#bbb", transition: "all var(--trans)" }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#bbb"; }}>
            <Icon name="edit" size={13} color="currentColor" strokeWidth={1.8} />
          </button>
          <button onClick={handleDelete} style={{ width: 28, height: 28, borderRadius: "var(--radius)", display: "flex", alignItems: "center", justifyContent: "center", color: "#bbb", transition: "all var(--trans)" }}
            ...
            onMouseLeave={e => { e.currentTarget.style.background = "transparent"; e.currentTarget.style.color = "#bbb"; }}>
            <Icon name="trash" size={13} color="currentColor" strokeWidth={1.8} />
          </button>
        </div>
      </div>
    </div>
              ...
              {STATUSES.map(s => {
                ...
                return (
                  <button key={s} type="button" onClick={() => setForm(p => ({ ...p, status: s }))}
                    style={{ padding: "9px 12px", borderRadius: "var(--radius-lg)", border: `1.5px solid ${active ? m.dot : "var(--mist2)"}`, background: active ? m.bg : "#fff", color: active ? m.color : "#888", fontSize: 12, fontWeight: active ? 700 : 500, display: "flex", alignItems: "center", gap: 7, transition: "all var(--trans)" }}>
                    <Icon name={m.icon} size={12} color={active ? m.color : "#bbb"} strokeWidth={2.2} />{s}
...
function Dashboard({ token, onLogout, showToast, initialUser }) {
  ...
  useEffect(() => { if (initialUser?.first_name) setUser(initialUser); }, []);
  ...
  const openBoard = (filter = "All") => { setBoardFilter(filter); setSearch(""); setBoardPage(1); setBoardOpen(true); };

  // Reset page on filter/search change
  useEffect(() => { setBoardPage(1); }, [boardFilter, search]);
  ...
  const paginatedBoard = filteredBoard.slice((boardPage - 1) * BOARD_PER_PAGE, boardPage * BOARD_PER_PAGE);
    ...
    <div style={{ minHeight: "100vh", background: "var(--paper)", display: "flex" }}>
      ...
      <aside style={{ width: 220, background: "#fff", borderRight: "1px solid var(--mist)", display: "flex", flexDirection: "column", padding: "22px 14px", position: "sticky", top: 0, height: "100vh", flexShrink: 0 }}>
        ...
        <button onClick={() => setProfileOpen(true)} style={{ display: "flex", alignItems: "center", gap: 10, width: "100%", padding: "10px 10px", borderRadius: "var(--radius-lg)", border: "1px solid var(--mist)", background: "var(--cream)", marginBottom: 24, transition: "all var(--trans)", textAlign: "left" }}
          ...
          <div style={{ flex: 1, minWidth: 0 }}>
            <div style={{ fontSize: 13, fontWeight: 700, fontFamily: "var(--font-display)", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap", color: "var(--ink)" }}>{user.first_name || "User"} {user.last_name || ""}</div>
      ...
      <main style={{ flex: 1, padding: "36px 40px", overflowY: "auto", minWidth: 0 }}>
        <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 32 }}>
          <div>
            <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: 28, letterSpacing: "-0.025em", lineHeight: 1, marginBottom: 6 }}>Hi, Welcome {user.first_name || "back"} 👋</h1>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            <button onClick={() => setProfileOpen(true)} style={{ display: "flex", alignItems: "center", gap: 8, padding: "8px 14px", borderRadius: "var(--radius-lg)", border: "1px solid var(--mist)", background: "#fff", fontSize: 13, fontWeight: 500, color: "var(--ink)", transition: "all var(--trans)" }}
              onMouseEnter={e => { e.currentTarget.style.borderColor = "var(--rust)"; e.currentTarget.style.color = "var(--rust)"; }}
              onMouseLeave={e => { e.currentTarget.style.borderColor = "var(--mist)"; e.currentTarget.style.color = "var(--ink)"; }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 7 }}>
                      ...
                      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                        <button onClick={() => openBoard(s)} style={{ fontSize: 11, color: m.color, fontWeight: 600, background: m.bg, border: `1px solid ${m.border}`, padding: "2px 8px", borderRadius: "99px", transition: "opacity var(--trans)" }}
                          ...
                          onMouseLeave={e => e.currentTarget.style.opacity = "1"}>{count} tasks</button>
          ...
          <div style={{ background: "#fff", border: "1px solid var(--mist)", borderRadius: "var(--radius-xl)", padding: "26px 28px" }}>
            ...
            {loading
              ...
              : recentTasks.length === 0
                ? <div style={{ textAlign: "center", padding: "32px 0", color: "#ccc", fontSize: 13 }}>No tasks yet — <button onClick={() => setModal({})} style={{ color: "var(--rust)", fontWeight: 600, textDecoration: "underline" }}>create one →</button></div>
          ...
          <div style={{ position: "fixed", top: 0, right: 0, bottom: 0, width: "min(700px,90vw)", background: "var(--paper)", borderLeft: "1px solid var(--mist)", zIndex: 201, display: "flex", flexDirection: "column", boxShadow: "-12px 0 48px rgba(10,10,15,0.14)", animation: "slideInPanel 0.28s cubic-bezier(0.4,0,0.2,1)" }}>
            ...
            <div style={{ padding: "20px 28px 16px", borderBottom: "1px solid var(--mist)", background: "#fff", display: "flex", alignItems: "center", gap: 14, flexShrink: 0 }}>
              ...
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                {["All", ...STATUSES].map(s => {
                  const m = s !== "All" ? STATUS_META[s] : null; const active = boardFilter === s;
                  return (
            ...
            <div style={{ padding: "14px 28px", borderBottom: "1px solid var(--mist)", background: "#fff", flexShrink: 0 }}>
              <div style={{ position: "relative", maxWidth: 380 }}>
                ...
                <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search tasks..."
                  ...
                  onBlur={e => { e.target.style.borderColor = "var(--mist2)"; e.target.style.boxShadow = "none"; }} />
                {search && (
                  <button onClick={() => setSearch("")} style={{ position: "absolute", right: 10, top: "50%", transform: "translateY(-50%)", color: "#bbb", display: "flex" }}>
              ...
              {loading
                ...
                : filteredBoard.length === 0
                  ? <div style={{ textAlign: "center", padding: "60px 24px", background: "#fff", borderRadius: "var(--radius-xl)", border: "1px solid var(--mist)" }}>
                    ...
                    <div style={{ fontSize: 13, color: "#bbb", marginBottom: 18 }}>{search ? `Nothing matches "${search}"` : ""}</div>
                  ...
                  : <>
                    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(272px,1fr))", gap: 14 }}>
                      {paginatedBoard.map(task => (
                        <TaskCard key={task.id} task={task} onEdit={t => setModal(t)} onDelete={deleteTask} onStatusChange={changeStatus} />
                      ))}
                    </div>
                    <Pagination page={boardPage} total={filteredBoard.length} perPage={BOARD_PER_PAGE} onChange={p => setBoardPage(p)} />
                  </>
              }
            </div>
          </div>
      ...
      {/* ── Profile panel ── */}
      {profileOpen && (
        <ProfilePanel user={user} setUser={setUser} token={token} onClose={() => setProfileOpen(false)} showToast={showToast} onLogout={() => { setProfileOpen(false); onLogout(); }} />
      )}

      {/* ── Task modal ── */}
      {modal !== null && (
        <TaskModal task={modal?.id ? modal : null} token={token} showToast={showToast} onClose={() => setModal(null)} onSave={() => { setModal(null); fetchTasks(); }} />
      )}
    </div>
  );
}
  ...
  return (
    <div style={{ minHeight: "100vh", background: "var(--cream)", position: "relative", overflow: "hidden" }}>
      ...
      <nav style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "18px 48px", borderBottom: "1px solid var(--mist)", position: "relative", zIndex: 10 }}>
        ...
        <div style={{ display: "flex", gap: 10 }}>
          <button onClick={onLogin} style={{ padding: "8px 20px", borderRadius: "var(--radius)", fontSize: 13, fontWeight: 500, color: "var(--ink)", border: "1px solid var(--mist2)", background: "transparent", transition: "border-color var(--trans)" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = "var(--ink)"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "var(--mist2)"}>Log in</button>
          <button onClick={onRegister} style={{ padding: "8px 20px", borderRadius: "var(--radius)", fontSize: 13, fontWeight: 600, color: "#fff", background: "var(--ink)", transition: "background var(--trans)" }}
            onMouseEnter={e => e.currentTarget.style.background = "var(--rust)"}
            onMouseLeave={e => e.currentTarget.style.background = "var(--ink)"}>Get started →</button>
        </div>
      </nav>
      <section style={{ maxWidth: 900, margin: "0 auto", padding: "100px 48px 80px", position: "relative", zIndex: 5 }}>
        <h1 style={{ fontFamily: "var(--font-display)", fontWeight: 700, fontSize: "clamp(46px,8vw,82px)", lineHeight: 1.05, letterSpacing: "-0.03em", marginBottom: 22, animation: "fadeUp 0.55s ease both" }}>
          Get things<br /><em style={{ color: "var(--rust)", fontStyle: "italic" }}>done</em> faster.
        </h1>
        ...
        <div style={{ display: "flex", gap: 12, animation: "fadeUp 0.55s ease 0.2s both" }}>
          <button onClick={onRegister} style={{ padding: "13px 32px", borderRadius: "var(--radius-lg)", fontSize: 15, fontWeight: 700, color: "#fff", background: "var(--rust)", transition: "all 0.2s" }}
            onMouseEnter={e => { e.currentTarget.style.background = "var(--rust-light)"; e.currentTarget.style.transform = "translateY(-1px)"; }}
            onMouseLeave={e => { e.currentTarget.style.background = "var(--rust)"; e.currentTarget.style.transform = "translateY(0)"; }}>Start for free</button>
          <button onClick={onLogin} style={{ padding: "13px 32px", borderRadius: "var(--radius-lg)", fontSize: 15, fontWeight: 500, color: "var(--ink)", background: "transparent", border: "1px solid var(--mist2)", transition: "border-color 0.2s" }}
            onMouseEnter={e => e.currentTarget.style.borderColor = "var(--ink)"}
            onMouseLeave={e => e.currentTarget.style.borderColor = "var(--mist2)"}>Sign in</button>
        </div>
      </section>
...
export default function App() {
  ...
  useEffect(() => {
    if (token) {
      ...
      if (saved) {
        setLoginUser(saved);
        if (saved.user_type === "admin") setView("admin");
  ...
  const handleLoginSuccess = (t, u) => {
    setToken(t); setLoginUser(u);
    if (u.user_type === "admin") setView("admin");
    else setView("dashboard");
  };
  ...
  return (
    <>
      ...
      {view === "admin"     && token && loginUser?.user_type === "admin" && (
        <AdminPanel token={token} adminUser={loginUser} onLogout={() => { clearToken(); setLoginUser(null); setView("landing"); showToast("Signed out", "info"); }} />
      )}
    </>
  );
}